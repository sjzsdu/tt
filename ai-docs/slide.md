...
`tt slide` 和 `tt markdown` 支持一个轻量扩展：Markdown 表格分隔行里的 `-` 数量会作为列宽比例提示。
...
上例中三列大约按 `1 : 1 : 3` 分配宽度。也可以写成 `| -- | --- | ------ |` 表示 `2 : 3 : 6`。冒号对齐仍可保留，例如 `| :-- | ---: | :------ |`，列宽只统计 `-` 的数量。

注意：这是 tt 渲染器的扩展，不是 CommonMark 标准。导出到其他 Markdown 渲染器时，列宽提示可能不会生效。
