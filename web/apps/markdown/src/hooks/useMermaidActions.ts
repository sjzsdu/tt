...
export function useMermaidActions(svg: string, svgElRef: RefObject<SVGSVGElement | null>, basename = 'mermaid-diagram') {
  const exportSvg = async () => {
    if (!svgElRef.current) return;
    const { svgToBlob, downloadBlob } = await import('../utils/export');
    downloadBlob(svgToBlob(svgElRef.current), `${basename}.svg`);
  };

  const exportPng = async () => {
    if (!svg) return;
    const { svgMarkupToPngBlob, downloadBlob } = await import('../utils/export');
    downloadBlob(await svgMarkupToPngBlob(svg), `${basename}.png`);
  };
