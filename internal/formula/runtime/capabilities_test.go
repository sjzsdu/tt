...
func TestBuildExternalAgentArgvAdapters(t *testing.T) {
	tests := []struct {
		...
		{name: "codex resume subcommand", driver: "codex", model: "m", resume: "s", want: []string{"codex", "exec", "--model", "m", "resume", "s"}},
		{name: "codex exec options precede resume", driver: "codex", model: "m", resume: "s", extra: []string{"--sandbox", "read-only"}, want: []string{"codex", "exec", "--model", "m", "--sandbox", "read-only", "resume", "s"}},
